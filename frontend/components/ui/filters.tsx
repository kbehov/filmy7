...
export const DEFAULT_I18N: FilterI18nConfig = {
  ...
  max: 'Max',
  to: 'do',
  typeAndPressEnter: 'Wpisz i naciśnij Enter, aby dodać',
  selected: 'wybrane',
  selectedCount: 'wybrane',
  percent: '%',
  defaultCurrency: '$',
  defaultColor: '#000000',
  addFilterTitle: 'Dodaj filtr',
  ...
  operators: {
    ...
    isNotAnyOf: 'nie jest jednym z',
    includesAll: 'zawiera wszystkie',
    excludesAll: 'wyklucza wszystkie',
    before: 'przed',
    after: 'po',
    between: 'między',
    ...
    includes: 'zawiera',
    excludes: 'wyklucza',
    includesAllOf: 'zawiera wszystkie z',
    includesAnyOf: 'zawiera dowolne z',
    empty: 'puste',
    notEmpty: 'niepuste',
  },
  ...
  // Placeholders
  placeholders: {
    enterField: (fieldType: string) => `Wpisz ${fieldType}…`,
    selectField: 'Wybierz…',
    searchField: (fieldName: string) => `Szukaj ${fieldName.toLowerCase()}…`,
  ...
  validation: {
    ...
    invalidTel: 'Nieprawidłowy format telefonu',
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-fit min-w-fit">
        {operators.map(op => (
          ...
          >
            <span>{op.label}</span>
            <CheckIcon className={cn('ms-auto text-primary', op.value === operator ? 'opacity-100' : 'opacity-0')} />
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
        ...
        >
          <ScrollArea className="size-full min-h-0 **:data-[slot=scroll-area-scrollbar]:m-0 **:data-[slot=scroll-area-viewport]:h-full **:data-[slot=scroll-area-viewport]:overscroll-contain">
            {allFilteredOptions.length === 0 && (
              <div className="py-2 text-center text-sm text-muted-foreground">{context.i18n.noResultsFound}</div>
            )}
        ...
        >
          <ScrollArea className="size-full min-h-0 **:data-[slot=scroll-area-scrollbar]:m-0 **:data-[slot=scroll-area-viewport]:h-full **:data-[slot=scroll-area-viewport]:overscroll-contain">
            {filteredOptions.length === 0 ? (
              <div className="py-2 text-center text-sm text-muted-foreground">{i18n.noResultsFound}</div>
            ) : (
              <DropdownMenuGroup>
