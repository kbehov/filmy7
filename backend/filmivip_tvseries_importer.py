...
VIDSRC_URL = "https://vsembed.ru/embed/tv"

# API Upload endpoint
API_UPLOAD_URL = "http://localhost:8081/api/v/1/movies/import-tv-series"
...
def check_episode_link(
    ...
    video_url = f"{VIDSRC_URL}?imdb={imdb_id}&season={season_number}&episode={episode_number}&ds_lang=bg"
    try:
...
def process_and_upload_tv_series(tv_id: int, season_number: int) -> bool:
    ...
    for actor in credits.get("cast", [])[:5]:
        ...
        if actor_details:
            ...
            time.sleep(0.1)  # Small delay between actor requests
    ...
    try:
        ...
        print(f"Uploading TV series: {tv_data['title']} - Season {season_number}")
